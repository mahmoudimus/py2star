...
class TypeInferenceEngine:
    ...
    def find_node(self, lnum, column):
        ...
        for node in ast.walk(self.tree):
            ...
            if node_lnum == lnum and node_col <= column <= node_col + len(
                    node_id):
                return node
        ...
        if key in AST_TYPES:
            ...
            for parent, kind, location in self.resolve(node):
                if isinstance(parent, ast.Assign):
                    ...
                    num_required = len(parent.args.args) - len(
                        parent.args.defaults)
                    if location >= num_required:
                        for result in self.evaluate(
                                parent.args.defaults[location - num_required]):
                            yield result
    ...
    def format(self, node):
        ...
        if isinstance(node, ast.Module):
            ...
            return '%s=%s' % (', '.join(self.format(t) for t in node.targets),
